...
export async function checkPremiumStatus(): Promise<boolean> {
  if (Purchases) {
    try {
      ...
      // エンタイトルメントIDが一致しない場合も、アクティブなものが1つでもあればプレミアムとみなす
      const isPremium =
...
export async function purchasePremium(): Promise<boolean> {
  ...
  try {
    ...
    // purchasePackage はキャンセル・エラー時に例外を投げる
    // 例外が発生しなければ購入成功（Appleが確認済み）
    ...
    // 購入成功 → エンタイトルメントIDの一致に関わらずプレミアムを付与
    // （IDが異なっていても Apple/RevenueCat が購入を確認しているため）
    await AsyncStorage.setItem(PREMIUM_CACHE_KEY, JSON.stringify({
      isPremium: true,
      checkedAt: Date.now(),
    }));
...
export async function restorePurchases(): Promise<boolean> {
  ...
  try {
    ...
    // エンタイトルメントIDが違っても、アクティブなものがあればプレミアムとみなす
    const isPremium =
      customerInfo.entitlements.active[ENTITLEMENT_ID] !== undefined ||
      Object.keys(customerInfo.entitlements.active).length > 0;
    await AsyncStorage.setItem(PREMIUM_CACHE_KEY, JSON.stringify({
